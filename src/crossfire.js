...
export async function initDatabase(request, env) {
  try {
    ...
    // 执行建表语句 - 注意：exec() 可能返回 undefined 或没有 duration 属性的对象
    const result = await env.db.exec(createTableSQL);
    
    // 创建索引（单独执行）
    await env.db.prepare("CREATE INDEX IF NOT EXISTS idx_email ON users(email)").run();
    await env.db.prepare("CREATE INDEX IF NOT EXISTS idx_status ON users(status)").run();
    await env.db.prepare("CREATE INDEX IF NOT EXISTS idx_create_time ON users(create_time)").run();
    
    return new Response(
      JSON.stringify({ 
        status: 'success', 
        message: 'Database table initialized successfully',
        // 避免访问可能不存在的 duration 属性
        result: result || 'exec completed'
      }), 
      { 
        status: 200,
        headers: { 
