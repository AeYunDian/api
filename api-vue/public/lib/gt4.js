...
(function (window) {
    ...
    function getTextZoom() {
        if (textZoomCache !== undefined) {
            return textZoomCache;
        }
        var fontSize = 20;
        var el = document.createElement('div');
        var root = document.body || document.documentElement;
        var renderFontSize = fontSize;
        ...
        if (window.getComputedStyle) {
            renderFontSize = window.getComputedStyle(el, null).fontSize;
        } else if (el.currentStyle) {
            renderFontSize = el.currentStyle.fontSize;
        }
        renderFontSize = parseFloat(renderFontSize);
        root.removeChild(el);
        if (!renderFontSize || isNaN(renderFontSize)) {
            renderFontSize = fontSize;
        }
        textZoomCache = renderFontSize / fontSize;
        return textZoomCache;
    }
    ...
    window.initGeetest4 = function (userConfig, callback) {
        ...
        if (!isNumber(config.fontScale) || config.fontScale <= 0) {
            config.fontScale = getTextZoom();
        }
        if (userConfig.https) {
