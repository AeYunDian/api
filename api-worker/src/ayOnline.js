...
async function initDatabase(db) {
  try {
    ...
    await db
      .prepare(
        `CREATE INDEX IF NOT EXISTS idx_feedbacks_user ON feedbacks(user_sub)`,
      )
      .run();
    await db
      .prepare(
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path.startsWith("/api/ayonline/")) {
        ...
        if (path === "/api/ayonline/revoke-oauth-tokens" && method === "POST") {
          const [authStatus, user] = await checkAuth(request, env);
          if (authStatus !== TAG_LOGGEDIN) {
            return jsonResponse({ error: "Unauthorized" }, 401, cors);
          }
       ...
       WHERE key LIKE ?
        ...
        if (path === "/api/ayonline/oauth-unbind" && method === "POST") {
          ...
          return jsonResponse(
            { success: true, message: `Unbound ${body.provider}` },
            200,
            cors,
          );
        }
        ...
        if (path === "/api/ayonline/update-profile" && method === "POST") {
          ...
          if (body.email) {
            ...
            const existing = await env.db
              .prepare(
                "SELECT sub FROM online_users WHERE email = ? AND sub != ?",
              )
          ...
          if (updates.email) {
            ...
            if (existing) {
              return jsonResponse(
                { error: "Email already exists", error_code: 1002 },
                409,
                cors,
              );
            }
          ...
          if (updates.username) {
            const existing = await env.db
              .prepare(
                "SELECT sub FROM online_users WHERE username = ? AND sub != ?",
              )
              .bind(updates.username, user.sub)
              .first();
            if (existing) {
              return jsonResponse(
                { error: "Username already exists", error_code: 1002 },
                409,
                cors,
              );
            }
          ...
          headers.append(
            "Set-Cookie",
            serialize("access_token", newAccessToken, cookieOptions),
          );
