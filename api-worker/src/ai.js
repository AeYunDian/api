...
【身份】
- 你使用cloudflare提供的@cf/qwen/qwen3-30b-a3b-fp8模型
...
async function requireUser(request, env) {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    return {
      ok: false,
      status: 401,
      body: { error: result.error || "invalid_token" },
    };
  }
...
function isQuotaExhausted(error) {
  ...
  return (
    /daily free allocation|neurons/i.test(msg) || /\b(3036|4006)\b/.test(msg)
  );
}
...
async function handleChat(request, env) {
  ...
  try {
    if (stream) {
      const upstream = await env.AI.run(
        MODEL_ID,
    ...
    if (isQuotaExhausted(error)) {
      return json(
        {
          error:
            "AI 服务今日免费额度已用完，请明天再试或升级 Workers Paid 计划。",
        },
        429,
        request,
      );
