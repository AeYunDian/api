export async function triggerWorkflow(env) {
    try {
        ...
        if (env.DEBUG) console.log(`Triggering workflow: ${OWNER}/${REPO}/${WORKFLOW_ID} on branch ${BRANCH}`);
        ...
        if (env.DEBUG) console.log(`Workflow triggered successfully at ${new Date().toISOString()}`);
        return new Response("Workflow triggered successfully", { status: 200 });
    } catch (err) {
        if (env.DEBUG) console.error(`Failed to trigger workflow: ${err.message}`);
